...
class RandomForest(object):
    ...
    def _bootstrapping(self, XX, n):
        ...
        #print np.count_nonzero(np.unique(idx)), "indexs in total"
        #print idx
        X=[];
        y=[];
        for i in idx:
    ...
    def fitting(self):
        ...
        for i in range(self.num_trees):
            ...
            X=np.array(self.bootstraps_datasets[i],dtype=float)
            y=np.array(self.bootstraps_labels[i],dtype=float)
            TREE.learn(X,y);
            #print len(TREE.tree)
...
def main():
    ...
    index=np.where(y_predicted!=-1)[0]
